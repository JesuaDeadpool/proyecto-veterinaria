...
route.post('/login', async (req, res) => {
    ...
    if (!usuario.correo_electronico || !usuario.contrasenia) {
        return res.status(400).json({ status: 400, message: `Correo electronico ${usuario.correo_electronico} o no y Contraseña son requeridos ${usuario.contrasenia}..` });
    }
    ...
    pool.query(sql, [usuario.correo_electronico], async (err, results) => {
        ...
        let user = results[0];
        const matching = await bcrypt.compare(usuario.contrasenia, user.contrasenia);
